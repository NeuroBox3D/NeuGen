 ...
 * a {@link #build(Point3dQH[]) build} method. After the hull is built, its
 ...
 *     { new Point3dQH(0.0, 0.0, 0.0), new Point3dQH(1.0, 0.5, 0.0), new Point3dQH(2.0, 0.0, 0.0),
 ...
 * 
 * <h3><a name=distTol>Robustness</h3>
 * Because this algorithm uses floating point arithmetic, it is potentially
 * vulnerable to errors arising from numerical imprecision. We address this
 * problem in the same way as <a href=http://www.qhull.org>qhull</a>, by
 * merging faces whose edges are not clearly convex. A face is convex if its
 * edges are convex, and an edge is convex if the centroid of each adjacent
 * plane is clearly <i>below</i> the plane of the other face. The centroid is
 * considered below a plane if its distance to the plane is less than the
 * negative of a {@link #getDistanceTolerance() distance tolerance}. This
 ...
 * 
 * <h3>Merged Faces</h3>
 * The merging of faces means that the faces returned by QuickHull3D may be
 * convex polygons instead of triangles. If triangles are desired, the
 * application may {@link #triangulate triangulate} the faces, but it should be
 * noted that this may result in triangles which are very small or thin and
 * hence difficult to perform reliable convexity tests on. In other words,
 ...
 * {@link #check check} will fail (the same behavior is observed with
 * triangulated output from <a href=http://www.qhull.org>qhull</a>).
 * 
 * <h3>Degenerate Input</h3>
 * It is assumed that the input points are non-degenerate in that they are not
 * coincident, colinear, or colplanar, and thus the convex hull has a non-zero
 * volume. If the input points are detected to be degenerate within the
...
public class QuickHull3D {
    ...
    /**
     * Specifies that (on output) vertex indices for a face should be listed
     * in clockwise order.
     */
    ...
    /** use to log messages */
    private static Logger logger = Logger.getLogger(QuickHull3D.class.getName());
     ...
     * computed hull. The distance tolerance is used to determine when faces
     * are unambiguously convex with respect to each other, and when points
     * are unambiguously above or below a face plane, in the presence of <a
     * href=#distTol>numerical imprecision</a>. Normally, this tolerance is
     * computed automatically for each set of input points, but it can be
     ...
     */
    public QuickHull3D() {
        logger.info("public QuickHull3D ()");
    }

    /**
     * Creates a convex hull object and initializes it to the convex hull of
     * a set of points whose coordinates are given by an array of doubles.
     *
     * @param coords
     *                x, y, and z coordinates of each input point. The
     *                length of this array will be three times the the
     ...
     *                 coplanar.
     */
    public QuickHull3D(Point3dQH[] points) throws IllegalArgumentException {
    ...
    protected void setFromQhull(double[] coords, int nump, boolean triangulate) {
        ...
        try {
            ...
            if (stok.ttype != StreamTokenizer.TT_NUMBER) {
                logger.info("Expecting number of faces");
                System.exit(1);
            }
    ...
    private void printPoints(PrintStream ps) {
        for (int i = 0; i < numPoints; i++) {
            Point3dQH pnt = pointBuffer[i].pnt;
            ps.println(pnt.x + ", " + pnt.y + ", " + pnt.z + ",");
        }
    }

    /**
     * Constructs the convex hull of a set of points whose coordinates are
     * given by an array of doubles.
     *
     * @param coords
     *                x, y, and z coordinates of each input point. The
     *                length of this array will be three times the number of
     *                input points.
     * @throws IllegalArgumentException
     *                 the number of input points is less than four, or the
     *                 points appear to be coincident, colinear, or
    ...
    public void build(double[] coords, int nump) throws IllegalArgumentException {
        if (nump < 4) {
            logger.error("Less than four input points specified",
                    new IllegalArgumentException("Less than four input points specified"));
        }
        if (coords.length / 3 < nump) {
            logger.error("Coordinate array too small for specified number of points",
                    new IllegalArgumentException("Coordinate array too small for specified number of points"));
        }
        initBuffers(nump);
        setPoints(coords, nump);
        buildHull();
     ...
     *                 coplanar.
     */
    public void build(Point3dQH[] points) throws IllegalArgumentException {
        //logger.info("public void build (Point3dQH[] points)");
     ...
     * @throws IllegalArgumentException
     *                 the number of input points is less than four or
     *                 greater then the length of <code>points</code>, or
     *                 the points appear to be coincident, colinear, or
     *                 coplanar.
     */
    public void build(Point3dQH[] points, int nump) throws IllegalArgumentException {
        if (nump < 4) {
            return;
        ...
        //logger.info("public void build (Point3dQH[] points, int nump)");
        //logger.info("nump: " + nump);
        initBuffers(nump);
     ...
     * and hence appear to be non-convex (this same limitation is present in
    ...
    protected void initBuffers(int nump) {
        System.out.println("protected void initBuffers (int nump)");
    ...
    protected void setPoints(Point3dQH[] pnts, int nump) {
        System.out.println("protected void setPoints (Point3dQH[] pnts, int nump)");
        for (int i = 0; i < nump; i++) {
            Vertex vtx = pointBuffer[i];
            vtx.pnt.set(pnts[i]);
            vtx.index = i;
    ...
    protected void computeMaxAndMin() {
        ...
        // System.out.println("max: x: " + max.x + " y: " + max.y + " z:
        // " + max.z);
        // System.out.println("min: x: " + min.x + " y: " + min.y + " z:
        // " + min.z);
    ...
    protected void createInitialSimplex() throws IllegalArgumentException {
        ...
        if (max <= tolerance) {
            ...
            logger.error("Input points appear to be coincident",
                    new IllegalArgumentException("Input points appear to be coincident"));
        }
        ...
        if (Math.sqrt(maxSqr) <= 100 * tolerance) {
            logger.error("Input points appear to be colinear", new IllegalArgumentException("Input points appear to be colinear"));
        ...
        if (debug) {
            logger.info("initial vertices:");
            logger.info(vtx[0].index + ": " + vtx[0].pnt);
            logger.info(vtx[1].index + ": " + vtx[1].pnt);
            logger.info(vtx[2].index + ": " + vtx[2].pnt);
            logger.info(vtx[3].index + ": " + vtx[3].pnt);
        }
     ...
     */
    public Point3dQH[] getVertices() {
        Point3dQH[] vtxs = new Point3dQH[numVertices];
     ...
     *
     * @param coords
     *                returns the x, y, z coordinates of each vertex. This
     *                length of this array must be at least three times the
     *                number of vertices.
    ...
    public int getVertices(double[] coords) {
        for (int i = 0; i < numVertices; i++) {
            Point3dQH pnt = pointBuffer[vertexPointIndices[i]].pnt;
            coords[i * 3 + 0] = pnt.x;
            coords[i * 3 + 1] = pnt.y;
            coords[i * 3 + 2] = pnt.z;
        }
    ...
    /**
     * Returns an array specifing the index of each hull vertex with respect
     * to the original input points.
     *
     ...
     *
     * <p>
     * Each face is represented by an integer array which gives the indices
     ...
     *
     * @return array of integer arrays, giving the vertex indices for each
     *         face.
     * @see QuickHull3D#getVertices()
     ...
     * <p>
     * Each face is represented by an integer array which gives the indices
     * of the vertices. By default, these indices are numbered with respect
     * to the hull vertices (as opposed to the input points), are
     * zero-based, and are arranged counter-clockwise. However, this can be
    ...
    public int[][] getFaces(int indexFlags) {
        //logger.info("public int[][] getFaces(int indexFlags)");
        ...
        for (Iterator it = faces.iterator(); it.hasNext();) {
            ...
            //System.out.println("face.numVertices() : " + face.numVertices());
     ...
     * followed by the vertex indices for each face (each preceded by the
     * letter <code>f</code>).
     *
     * <p>
     * The face indices are numbered with respect to the hull vertices (as
     * opposed to the input points), with a lowest index of 1, and are
     * arranged counter-clockwise. More control over the index format can be
     * obtained using {@link #print(PrintStream,int) print(ps,indexFlags)}.
     *
     * @param ps
     ...
     * vertices (as opposed to the input points), with a lowest index of 1,
     * and are arranged counter-clockwise. However, this can be changed by
     * setting {@link #POINT_RELATIVE POINT_RELATIVE},
    ...
    private boolean doAdjacentMerge(Face face, int mergeType) {
        ...
        do {
            ...
            { // merge faces if they are parallel or
                // non-convex
    ...
    protected void calculateHorizon(Point3dQH eyePnt, HalfEdge edge0, Face face, Vector<HalfEdge> horizon) {
        // oldFaces.add (face);
    ...
    protected void addPointToHull(Vertex eyeVtx) {
        ...
        // for (Face face = newFaces.first(); face != null; face =
        // face.next)
        // {
        // if (face.mark == Face.VISIBLE)
        // {
    ...
    protected void buildHull() {
        logger.info("protected void buildHull ()");
        int cnt = 0;
        Vertex eyeVtx;

        computeMaxAndMin(); // min and max point
    ...
    protected void reindexFacesAndVertices() {
        ...
        for (Iterator it = faces.iterator(); it.hasNext();) {
            ...
            if (face.mark != Face.VISIBLE) {
                //System.out.println("1 face.getVertexString()" + face.getVertexString());
                it.remove();
            } else {
                //System.out.println("2 face.getVertexString()" + face.getVertexString());
     ...
     * than <i>tol</i> above the plane of the other face. Similarly, a
     * point is considered outside a face if its distance to that face's
     * plane is more than 10 times <i>tol</i>.
     *
     * <p>
     * If the hull has been {@link #triangulate triangulated}, then this
