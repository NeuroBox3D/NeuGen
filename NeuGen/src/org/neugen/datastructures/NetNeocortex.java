...
public final class NetNeocortex extends NetBase implements Serializable, Net {
	...
	/**
	 * Function for interconnecting the net for a cortical column. It
	 * connects the neurons with each other and computes the number of
	 * synapses.
	 */
	@Override
	...
	/**
	 * @brief get the NGX data for export
	 * @return WriteToNGX
	 */
	@Override
	...
	/**
	 * @brieg get the txt data for export
	 * @return 
	 */
	@Override
	...
	public class WriteTXTData implements WriteToTXT {
		...
		public float get_uEPSP_Value(int typeN1, int typeN2) {
			/// unitary EPSP variable for the various neuron type combinations
			float uEPSP = -1.0000f; 
		...
		public void writeAlphaSynapses(NeuronTypes neuronType, CompressorOutputStream cos, PrintWriter pw) {
			...
			for (int j = 0; j < synapseList.size(); j++) {
				...
				/// debug
				/// System.err.println("ax_length: " + ax_section.getLength());
				/// System.err.println("relative_to_walk: "+ dd);
				float to_walk_on_given_edge_axon = 0;
				int index = 0;
				float walked = 0f;
				for (int i = 0; i < ax_section.getSegments().size() - 1; i++) {
	...
	public class WriteToHocData implements WriteToHoc {
		...
		public final void writetohocExp2Synapses(Writer fw, Writer synFW) throws IOException {
			...
			for (int j = 0; j < synapseList.size(); j++) {
				 ...
				 * @note to use ax_local_position is a bug,
				 * since the ax_local_position is not in the
				 * range [0,1] - by definition of
				 * ax_local_position (cf. above!)
				 *
				 * instead of using ax_local_position, we are
				 * required to know the relative position within
				 * the given section, where the synaptic
				 * connection should end (i. e. end
				 * vertex/coordinates), which is by definition
				 * the corresponding dendrite to the axon at
				 * hands (see above Exp2Syn output, where the
				 * corresponding axon is used!)
				 *
				 * ff specified the relative location within
				 * bounds [0,1] on the given dendrite.
				 *
				 */
