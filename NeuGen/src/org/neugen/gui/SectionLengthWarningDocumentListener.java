...
/**
 * @brief warns if section length is too high for reasonable large-scale networks
 * This means, if the section_length is too high, we except many vertices which are
 ...
 */
public final class SectionLengthWarningDocumentListener implements DocumentListener {
	/// private final members
	private final JLabel warningLabel;
	
        /// private static final members
        private static final Float maxSectionLength = 10.f;
        private static final String warningMessage = "Section way too high";
        private static final String warningMessageTooltip = "For large scale-networks " 
                + "a density below " + maxSectionLength + " is recommended to avoid double "
	 ...
	 * @param warningLabel
	 */
	public SectionLengthWarningDocumentListener(final JLabel warningLabel) {
	...
	private void warn(DocumentEvent e) {
		try {
			...
			if (!text.isEmpty()) {
				if (Float.parseFloat(text) > maxSectionLength) {
